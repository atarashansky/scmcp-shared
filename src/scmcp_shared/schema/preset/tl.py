...
class ListInferenceMethodParam(BaseModel):
    """ListInferenceMethodParam"""

    pass


class ULMParam(BaseModel):
...
class PathwayActivityParam(MLMParam):
    """Input schema for decoupler's Multivariate Linear Model (MLM) method.
    ...
    organism: str = Field(
        default="human", description="The organism of interest. By default human."
    )
    top: int | float = Field(
        default=float("inf"),
    ...
    thr_padj: float = Field(
        default=0.05, description="Significance threshold to trim interactions."
    )
    license: str = Field(
        default="academic",
...
class TFActivityParam(ULMParam):
    """Input schema for decoupler's Univariate Linear Model (ULM) method.
    ...
    remove_complexes: bool = Field(
        default=False, description="Whether to remove complexes."
    )
    license: str = Field(
        default="academic",
        description="Which license to use, available options are: academic, commercial, or nonprofit.",
